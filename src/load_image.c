...
/*   Updated: 2021/12/10 16:23:28 by gcosta-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	load_image_enemy(t_global *global)
{
	global->data->img_enemy_front = mlx_xpm_file_to_image(\
		global->data->mlx_ptr, ENEMY_FRONT, &(global->data->img_width), \
		&(global->data->img_height));
	global->data->img_enemy_back = mlx_xpm_file_to_image(\
		global->data->mlx_ptr, ENEMY_BACK, &(global->data->img_width), \
		&(global->data->img_height));
	global->data->img_enemy_right = mlx_xpm_file_to_image(\
		global->data->mlx_ptr, ENEMY_RIGHT, &(global->data->img_width), \
		&(global->data->img_height));
	global->data->img_enemy_left = mlx_xpm_file_to_image(\
		global->data->mlx_ptr, ENEMY_LEFT, &(global->data->img_width), \
		&(global->data->img_height));
}
